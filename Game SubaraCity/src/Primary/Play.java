...
/* main program
 * class Game,Button and Menu not use
 * 
 */
...
//	private Button menu;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		{
			xa.set(i, xa.get(i)+dx.get(i)*4);
			ya.set(i, ya.get(i)+dy.get(i)*4);
		}
		time1stop();
		repaint();
	}
	...
	public Play() {
		...
		time1 = new  Timer(15, this);
		time2 = new Timer(10,new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				for(int i=0;i<ya.size();i++)
					if(ya.get(i)<=dy.get(i)) ya.set(i, ya.get(i)+10);
					else {
...
//			return;
//		}
		if(xa == null) System.out.println("null here");
//		xa = new ArrayList<Integer>();
		for(int i=0;i<xa.size();i++)
				switch(dz.get(i)) {
		...
		for(int i=0;i<5;i++)
			for(int j=0;j<5;j++) {
				...
				if(p == null) System.out.println("picture null");
				switch(getheight) {
